...
class UmkmViewMatriksProdukPage extends StatelessWidget {
  ...
  Future<List<SjhMatriksItem>> _getMatriks({
    ...
    try {
      ...
      final matriksList = res.data['matriks_produk'].map<SjhMatriksItem>((json) => SjhMatriksItem.fromJSON(json)).toList();
      return matriksList;
    }
  ...
  Widget _getMatriksTable({
    ...
    final listProduk = matriks[0]?.listBahan.map((bahan) => bahan.barang).toList() ?? List.empty();
          ...
          ...matriks.map((matrik) => TableRow(
            children: [
              ...
              ...listProduk.map((produk) => TableCell(
                child: Container(
                  padding: EdgeInsets.all(10),
                  child: matrik.listBahan.firstWhere((element) => element.barang == produk).status ?? false
            ...
            builder: (context, AsyncSnapshot snapshot) {
              if (snapshot.hasData) {
                ...
                return Container(
                  ...
                  child: _getMatriksTable(matriks: matriksList, context: context)
                );
              }
